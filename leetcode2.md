	...
	vector<int> inorderTraversal(TreeNode* root) {
		...
		while (cur || !s.empty())
		{
			...
			else
			{
				...
				// 两种情况，若右为空，则下一步继续处理根节点（因为是从左下往上处理，无需再考虑左节点）
...
最后https://leetcode-cn.com/problems/binary-tree-inorder-traversal/solution/wan-quan-mo-fang-di-gui-bu-bian-yi-xing-miao-sha-q/。上述链接中的模板可直接写出三种排列。对于后序，由于每次最先访问到根节点，但需要最后才pop，故不妨将根节点先入栈，且用NULL来标识这是根节点，当遇到NULL时，说名该pop根节点了。。
...
分析：利用广度优先搜索法。。本题中的限制条件为：不能移出放格外，且不能进入行坐标加列坐标数位之和大于k的格子。。。要求是求得走过多少格子。。。。那么本题可将限制条件当成一堵墙，需要求得墙里面的格子数。。。。很明显，**涉及到上下左右行走的题就需要使用bfs/dfs**，，那么定义dfs(0,0,m,n)为该区域内可行走到的格子数，在此基础上按模板进行代码编写。
...
public:
    ...
    */
    ...
    // i为当前机器人所处位置的横坐标，j为纵坐标。 m，n则为方格的最大横纵坐标
    // dfs的含义为，机器人从（i，j）到（m，n）可经历的格子数
...
分析：dfs的定义为，board中以ij为起始位置，是否与word中以m起始文字一致

```c++
class Solution {
public:
    bool dfs(int i,int j,vector<vector<char>>& board, string word,vector<vector<bool>>& used,int m) {
        if(i<0 || i> board.size()-1 || j<0 || j> board[0].size()-1 || used[i][j] || word[m] != board[i][j]) return 0;
        if(m == word.size()-1) return 1;

        used[i][j] = 1;
        bool flag = dfs(i+1,j,board,word,used,m+1) ||
                    dfs(i-1,j,board,word,used,m+1) ||
                    dfs(i,j-1,board,word,used,m+1) ||
                    dfs(i,j+1,board,word,used,m+1);
        used[i][j] = 0;
        return flag;
    ...
    bool exist(vector<vector<char>>& board, string word) {
        ...
        for(int i=0;i<board.size();i++){
            for(int j=0;j<board[0].size();j++){
                flag = dfs(i,j,board,word,used,0);
                if(flag) return 1;
            }
        }
        return 0;
...
void display(const vector<int> &nums,int n) {
	for (int i = 1; i <= n;i++) {
		cout << nums[i] << " ";
	}
	cout << endl;
...
bool isfit(int loc,int n,int num,vector<int>nums) {
	if (loc == 1) return 1;
	if (loc == n) return iszhishu(nums[1], num) && iszhishu(nums[loc - 1], num);
	return iszhishu(nums[loc - 1], num);
}
// dfs代表在第loc个位置放置合适的元素
void dfs(int loc, int n, vector<int> &nums, vector<bool> &used) {
	// 若放置完了n个位置，则说明已完成要求，则输出
	if (loc == n + 1) {
		display(nums,n);
	}

	for (int i = 1; i <= n; i++) {
		if (used[i]) continue;     // 若该数已被占用，则继续
		if (isfit(loc,n, i, nums) == 0) continue;
		used[i] = 1;
		nums[loc] = i;
		dfs(loc + 1, n, nums,used);
		used[i] = 0;
...
public:
	...
	bool dfs(vector<vector<char>>& board, int p, int q, string word, int index, vector<vector<bool>>& used)
	{
		if (index >= word.size()) 
			return 1;
...
![image-20201110212021114](C:\Users\a\AppData\Roaming\Typora\typora-user-images\image-20201110212021114.png)

分析：看到此类题首先想到的就是利用dfs求解。本题中与之前做的占位（素数环、N皇后）与行走不太一样，需要先转换一下题目。。本题要求求岛屿数目，岛屿是由0包围的，那么本题可以从0、0位置开始遍历，如果为1，则岛屿数加1并向上下左右行走，将本次可接触的所有1均置为0；如果为0，则说明不可能是岛屿，直接进行下次遍历即可。

```c++
class Solution {
public:
    void dfs(int i,int j,vector<vector<char>>& grid){
        if(i<0 || i>grid.size()-1 || j<0 || j > grid[0].size()-1 || grid[i][j]=='0') return;
    ...
    int numIslands(vector<vector<char>>& grid) {
        ...
        vector<vector<int> > used(col,vector<int>(line,0));
...
分析：若是一行数据，然后求最长连续字串的最大和，假设前i个元素的最大和为dp[i]，dp[i]的状态方程为，若dp[i-1]<0，则dp[i] = num[i]，日票dp[i-1]>0,则dp[i] = dp[i-1]+num[i]。。。那么本题其实可以将各连续行组合一下，然后再按照一维进行计算。
...
![image-20201111194646724](C:\Users\a\AppData\Roaming\Typora\typora-user-images\image-20201111194646724.png)

分析：举一个例子1，3，2，7，5，9，8，2。想要找下一个更大的排列就需要从最后一位开始，依次与前一位数字比较，直到找到前面数字小于该位数字停止，并标记此数字，，例如上述例子中，5<9，数字5对应下标为4.。。那么下一步我们需要从5后找一位刚好大于5的数字进行交换，5<8，故5和8交换位置，变为1，3，2，7，8，9，5，2.。最后再将下标4之后的数字从小到大排列，即变为1 3 2 7 8 2 5 9
...
题目要求统计原子数量，由于可能会在不同的位置出现一样的原子，故为便于统计，用map（有序）。且此题需通过分类讨论进行，即出现括号和未出现括号的情形。。



```c++
入栈的时候不需要送入map，但需要将原子数量也送入栈。
出栈的时候需判断栈顶是否为数字，若为数字，则mp[c] += (数字乘以括号外的数字)
							若不为数字，则mp[c] += (括号外的数字)
（错）错误点在于，当出现多层括号时，内存括号的原子也与外层数目有关，而下列代码中只是简单的将最内层利用栈率先弹了出来，并未考虑外层的数目。
#include"pch.h"
#include<iostream>
#include<math.h>
#include<vector>
...
public:
	...
	// 判断a是否在bc之间
	bool Isfit(char a, char b, char c) {
		if (a >= b && a <= c) return 1;
		return 0;
	}
	// 获取原子长度
	int Getatomlen(string formula, int i, int len) {
		int start = i;
		if (i == len - 1) return 1;		
		i++;

		while (i < len - 1 && Isfit(formula[i ], 'a', 'z')) i++;
		return (i - start);
	}
	// 计算原子数目
	int Getatomnum(string formula, int i, int len) {
		int start = i;
		if (i == len || !Isfit(formula[i], '0', '9')) return 1;
		while (i < len && Isfit(formula[i], '0', '9')) i++;
		return stoi(formula.substr(start, i - start));
	}

	// 计算int是几位数,若返回值为0，则说明原子数为1
	int Getshuwei(int n) {
		if (n == 1) return 0;
		int resnum = 1;
		while (1) {
			n /= 10;
			if (n == 0) return resnum;
			resnum++;
		}
	...
	string countOfAtoms(string formula) {
		...
		for (int i = 0; i < len; i++) {
			if (Isfit(formula[i], 'A', 'Z')) {
				int atom_len = Getatomlen(formula, i, len);
				int atom_num = Getatomnum(formula, i+ atom_len, len);
				mp[formula.substr(i, atom_len)] += atom_num;
				i += (atom_len + Getshuwei(atom_num)-1);
			}
			...
			if (formula[i] == '(') {
				...
				while (!st.empty()) {
					...
					// 入栈操作，入数字，原子与左括号
						...
						st.push(formula.substr(i, atom_len));
						st.push(to_string(atom_num));

						i += (atom_len + Getshuwei(atom_num));
					}

					else if(i < len && formula[i] == ')') {

						// 遇到右括号，肯定有数字
						int num = Getatomnum(formula, i + 1, len);
						i += (Getshuwei(num) + 1);
						while (st.top() != "(") {
							...
							mp[ss] = (mp[ss])+ stoi(s_num)* num;
							st.pop();   // 在空之前，肯定会遇到左括号，所以不担心是否无法pop
						}
						st.pop();
					}
		...
		for (map<string, int> ::iterator it = mp.begin(); it != mp.end(); it++) {
			res.append((*it).first);
			string tmp = to_string((*it).second);
			if (tmp == "1") continue;
...
{
	Solution s1;
	cout<<s1.countOfAtoms("K4(ON(SO3)2)2");
...
并查集主要用来划分圈子，比如10个人，认识的人为一个圈子，一个可划分为几个圈子。思路：1、先将10个人均划分为10个圈子。2、合并，将认识的人划分在一个圈子   ，在合并时，需找到一个人的根节点 ，再将新人连接到该根节点上，形成一个圈子。
